async function initializeDatabase() {
  try {
    ...
    switch (err.code) {
      ...
      case "ECONNREFUSED":
        throw new Error(
          "Check your connection details (packages/template/settings.json) or make sure your MySQL server is running."
        );
      case "ER_BAD_DB_ERROR":
        throw new Error(
          "The database name you've entered does not exist. Ensure the details inside your config file are correct (packages/template/config.json)."
        );
      case "ER_ACCESS_DENIED_ERROR":
        throw new Error(
          "Check your MySQL username and password and make sure they're correct."
        );
      case "ENOENT":
        throw new Error(
...
if (mp.settings.db_debug) {
  ...
  mp.db.on("enqueue", function () {
    console.log(
      `${"[WARNING] You have hit the database connection limit and queries are waiting to be executed."}`
    );
  });
